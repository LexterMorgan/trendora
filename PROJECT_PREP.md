...
Milestone 25A adds an isolated `FacebookPublicClient` (`src/trendora/connectors/facebook/`) retrieving public Page posts and visible engagement (reactions/comments/shares) from an explicit Page ID, via mocked HTTP only. Milestone 25B adds `normalize_facebook_posts` mapping those posts into `ResearchReference` + extended `ResearchMetrics` (`reaction_count`/`share_count`) and grounded evidence (`REACTION_COUNT`/`SHARE_COUNT` → raw_metrics). Facebook remains isolated and unavailable through `/api/v1/research` and `/api/v1/research/report`. No OAuth, token persistence, keyword search, database, API wiring, or live Meta credentials — live compatibility remains unverified. Backend unit suite: **812 passing** (795 + 17 new). No forecast tables, Streamlit, or ML libraries.
