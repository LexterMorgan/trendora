...
Milestone 13 adds the deterministic research contracts: `ResearchQuery`, platform capability vocabulary + static source declarations, `ResearchCapabilityResolver` coverage resolution (available/unavailable/conditional with reasons; complete/partial/none), and the synchronous in-memory `ResearchRun` lifecycle. No retrieval, no DB, no API, no connectors, no LLM, no new dependencies. Full suite: **392 passing** (354 + 38 new). No forecast tables, Streamlit, or ML libraries.
