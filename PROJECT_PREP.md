...
## Current status (Milestone 26E — 2026-09-08)

Milestones 2A–4 ingestion, 5 (analytics), 6A–6C (forecasting baselines/eval/comparison), 7 (series diagnostics), 8 (product contract), 10 (V1 GitHub forecast product), 11B (FastAPI adapter), 13 (research core), 14 (YouTube-first retrieval), 15 (research API), 16 (research workspace UI), 17 (evidence + observations), 18 (pattern aggregation), 19 (grounded interpretation contract), 20 (AI provider adapter), 21 (gaps & opportunities), 22 (ideas & briefs), 23A (report pipeline + API), M24.2 (frontend entry experience), and **M25A–M25E (Facebook public Page client → normalization → research execution → API wiring → frontend research mode)** are implemented. Milestone 6B remains the evaluation/model-selection write-up in [docs/06_ML_FORECASTING.md](docs/06_ML_FORECASTING.md). Milestone 9 decides the V1 forecasting product requirements in [docs/12_FORECASTING_PRODUCT_REQUIREMENTS.md](docs/12_FORECASTING_PRODUCT_REQUIREMENTS.md). Milestone 11A/M11B define and implement the forecast API contract ([docs/13](docs/13_FORECASTING_API_CONTRACT.md), `src/trendora/api/`). Milestone 12 re-baselines the product direction in [docs/14_PRODUCT_ARCHITECTURE_REBASELINE.md](docs/14_PRODUCT_ARCHITECTURE_REBASELINE.md).

Milestone 25A adds an isolated `FacebookPublicClient` (`src/trendora/connectors/facebook/`) retrieving public Page posts and visible engagement (reactions/comments/shares) from an explicit Page ID, via mocked HTTP only. Milestone 25B adds `normalize_facebook_posts` mapping those posts into `ResearchReference` + extended `ResearchMetrics` (`reaction_count`/`share_count`) and grounded evidence (`REACTION_COUNT`/`SHARE_COUNT` → raw_metrics). Milestone 25C adds explicit single-Facebook-Page execution through the domain/application/report seams (`ResearchQuery.facebook_page_id`, `FacebookResearchRetriever`, generic retriever protocol). Milestone 25D wires Facebook into the FastAPI read model: optional `META_ACCESS_TOKEN`/`META_GRAPH_API_VERSION` settings, `ExitStack`-owned client lifecycle (exactly-once close, including build failures), 503/502 error mapping, and `facebook_page_id` + `reaction_count`/`share_count` request/response fields. Explicit Facebook requests fail with 503 `research_source_not_configured` when the connector is unconfigured — never a silent empty success. Milestone 25E adds the frontend Facebook Page research mode: a single-source radio selector (YouTube default, Facebook opt-in, Instagram/TikTok visibly disabled), a required public Page-ID input with no end-user Facebook login, source-aware reference cards and turn summaries, and source-neutral error wording — no new dependencies, no live Meta verification. Milestone 26B adds combined YouTube + Facebook backend execution: mixed-source queries with deterministic `divmod` limit allocation across executable sources, per-source capability resolution without cross-product, fail-closed unconfigured sources before any network call, merged in-order references, and one evidence pack/AI pass per combined report. Milestone 26C makes `markets` (an ordered, normalized, deduplicated SEA market list) the canonical query field with truthful per-reference `market_contexts`, per-market YouTube retrieval targets, and fail-closed limit allocation; Milestone 26D replaces the single-market dropdown with a multi-select checkbox group (fixed-order submission, ≥1 market, default `["SG"]`) and the docs are synced in [docs/32_MULTI_MARKET_RESEARCH.md](docs/32_MULTI_MARKET_RESEARCH.md). No OAuth, token persistence, keyword search, database, or live Meta credentials — live compatibility remains unverified. Backend unit suite: **925 passing** (893 pre-M26C baseline + 32 M26C multi-market tests). No forecast tables, Streamlit, or ML libraries.
