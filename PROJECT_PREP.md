...
## Current status (Milestone 25A — 2026-09-01)

Milestones 2A–4 ingestion, 5 (analytics), 6A–6C (forecasting baselines/eval/comparison), 7 (series diagnostics), 8 (product contract), 10 (V1 GitHub forecast product), 11B (FastAPI adapter), 13 (research core), 14 (YouTube-first retrieval), 15 (research API), 16 (research workspace UI), 17 (evidence + observations), 18 (pattern aggregation), 19 (grounded interpretation contract), 20 (AI provider adapter), 21 (gaps & opportunities), 22 (ideas & briefs), 23A (report pipeline + API), M24.2 (frontend entry experience), and **M25A (Facebook public Page client)** are implemented. Milestone 6B remains the evaluation/model-selection write-up in [docs/06_ML_FORECASTING.md](docs/06_ML_FORECASTING.md). Milestone 9 decides the V1 forecasting product requirements in [docs/12_FORECASTING_PRODUCT_REQUIREMENTS.md](docs/12_FORECASTING_PRODUCT_REQUIREMENTS.md). Milestone 11A/M11B define and implement the forecast API contract ([docs/13](docs/13_FORECASTING_API_CONTRACT.md), `src/trendora/api/`). Milestone 12 re-baselines the product direction in [docs/14_PRODUCT_ARCHITECTURE_REBASELINE.md](docs/14_PRODUCT_ARCHITECTURE_REBASELINE.md).

Milestone 25A adds an isolated `FacebookPublicClient` (`src/trendora/connectors/facebook/`) retrieving public Page posts and visible engagement (reactions/comments/shares) from an explicit Page ID, via mocked HTTP only. Milestone 25B adds `normalize_facebook_posts` mapping those posts into `ResearchReference` + extended `ResearchMetrics` (`reaction_count`/`share_count`) and grounded evidence (`REACTION_COUNT`/`SHARE_COUNT` → raw_metrics). Milestone 25C adds explicit single-Facebook-Page execution through the domain/application/report seams (`ResearchQuery.facebook_page_id`, `FacebookResearchRetriever`, generic retriever protocol). Facebook remains isolated and unavailable through `/api/v1/research` and `/api/v1/research/report`. No OAuth, token persistence, keyword search, database, API wiring, or live Meta credentials — live compatibility remains unverified. Backend unit suite: **828 passing** (812 + 16 new). No forecast tables, Streamlit, or ML libraries.
